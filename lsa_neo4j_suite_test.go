...
var _ = Describe("Driver", func() {
	...
	It("Post to database", func() {
		...
		eids, _ = SaveGraph(session, tx, grph, cfg, 0)
	})
	...
	It("Load from database", func() {
		...
		for nodeItr := grph.GetNodes(); nodeItr.Next(); { // 80
			ix := 0
			// node := nodeItr.Node()
			// node.SetLabels(graph.NewStringSet(fmt.Sprintf("ix%d", ix)))
		...
		for edgeItr := grph.GetEdges(); edgeItr.Next(); { // 80
			edge := edgeItr.Edge()
			edgeSources = append(edgeSources, edge)
		}
		for edgeItr := expectedGraph.GetEdges(); edgeItr.Next(); { // 80
			edge := edgeItr.Edge()
		...
		for g := range gotSources {
			...
			for e := range expectedSources {
				eq := graph.CheckIsomorphism(gotSources[g].GetGraph(), expectedSources[e].GetGraph(), func(n1, n2 graph.Node) bool {
					...
					// if n1.GetLabels().Has(fmt.Sprintf("ix%d", g)) {
					// 	if n2.GetLabels().Has(fmt.Sprintf("ix%d", e)) {
					// 		return true
					// 	}
					// 	return false
					// }
